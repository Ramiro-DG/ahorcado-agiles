...
Given("el jugador inicia un nuevo juego de ahorcado", async function () {
  await page.locator("text/Start Testing").click();
  good_index = 0;
  bad_index = 0;
});
...
When("el jugador adivina una letra {string}", async function (string) {
  const letter =
    string === "correcta"
      ? letras_correctas[good_index++]
      : letras_incorrectas[bad_index++];
  await page.locator(`button[id="${letter}"].button.is-primary.m-1`).click();
...
Then(
  ...
  async function (string) {
    ...
    if (string !== "no") {
      assert(text_shown.includes(letras_correctas[good_index - 1]));
    } else {
      assert.equal(
...
Then(
  ...
  async function (int) {
    ...
    const text_shown = await page.evaluate(
